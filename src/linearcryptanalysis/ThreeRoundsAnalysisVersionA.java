 ...
 */
public class ThreeRoundsAnalysisVersionA {
    ...
    public ThreeRoundsAnalysisVersionA(int[] key, int num) {
        this.key = key;
        this.numOfPairs = num;
        this.counter = 0;
    ...
    public ThreeRoundsAnalysisVersionA(int numKeysForStat, int num) {
        this.numOfPairs = num;
        this.numOfKeys = 65536;
   ...
   //two active S-boxes S+L
    //2e80 -> 006c -> b36e
    private boolean bitXor(int u1, int u2) {
        boolean result = false;
        boolean u1Xor = getBitAtPos(u1, 0)^getBitAtPos(u1, 1)^getBitAtPos(u1, 4)^getBitAtPos(u1, 5)^getBitAtPos(u1, 7);
        boolean u2Xor = getBitAtPos(u2, 1) ^ getBitAtPos(u2, 2)^ getBitAtPos(u2, 3)^ getBitAtPos(u2, 5)^ getBitAtPos(u2, 6);

        //bit xor of chosen bits of plaintext, u1 and u2
        result = getBitAtPos(plainText[1], 1) ^ getBitAtPos(plainText[1], 2) 
                ^ getBitAtPos(plainText[1], 3) ^ getBitAtPos(plainText[1], 5) ^ getBitAtPos(plainText[3], 7)              
                ^ u1Xor ^ u2Xor;
